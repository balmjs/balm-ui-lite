<template>
  <div class="page--confirm">
    ...
    <h4>其中<strong>message</strong>参数可以是一个对象，其属性和全局预设属性几乎完全一致，但是多出了一个defaultValue属性，该属性将用于设置提示框表单默认值；且当<strong>message</strong>参数为一个对象时，$prompt方法会忽略第二个参数[default]。</h4>
    ...
    <h4>这是使用Prompt插件来处理逻辑的标准方式，但是由于$prompt方法返回的是一个Promise实例，所以它还有另外两种写法：</h4>
    <ui-markdown :text="code[3]"></ui-markdown>
    <h4>你可以比较几种写法的区别。如果你不需要复写提示框的样式，我们强烈推荐使用最后一种方式。这将会和window.prompt用法保持一致。</h4>
  </div>
</template>
